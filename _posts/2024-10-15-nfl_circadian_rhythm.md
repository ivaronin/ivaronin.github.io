...
We will then have to clean and transform the data to prepare it to be used to investigate the effects circadian on NFL game outcomes.

We will work in python which has a number of helpful libraries for a project like this, notably `requests` and `BeautifulSoup` to help with scraping data and `pandas` to help with analysis, among others.
